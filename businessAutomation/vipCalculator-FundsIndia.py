...
class vipInvestment:
    ...
    def findRet(self):
        ...
        instAmount   = self.initInvest; #Assuming intialial Investment = Installment Amount
        count        = 0
        shareOwn     = 0
        totalInvst   = 0
        prevTargetPV = 0
        for nav in self.navFile:
            ...
            currTargetPV  = (prevTargetPV * (self.expectedRet/100.0))/12.0 \
                             + (prevTargetPV + instAmount)
            shareReq      = currTargetPV/navFloat
            ...
            #print
            self.__printRow(navFloat,currTargetPV,shareOwn,shareBro,currInvst)
            #Exit condition of Loop
            if ((totalInvst < currTargetPV) and
                (count>= self.installmentCount)):
                break
            else:
                prevTargetPV = currTargetPV
